...
;; Don't require scientific-mapping here as it's loaded by the package entry point
;; Making these requires optional to avoid circular dependencies
(require 'core/mode nil t)
(require 'ai-integration nil t)
(require 'academic-apis nil t)
(require 'reference-managers nil t)
(require 'page-view nil t)
...
(defvar scientific-mapping-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-c s C") 'doc-engine-capture)
    (define-key map (kbd "C-c s y") 'doc-engine-capture-with-yasnippet)
    ...
    ;; Page view and dashboard
    (define-key map (kbd "C-c s H") 'scientific-dashboard-open)
    (define-key map (kbd "C-c s l") 'page-view-start-writing-session)
    (define-key map (kbd "C-c s L") 'page-view-with-timeline)
    (define-key map (kbd "C-c s M-l") 'page-view-without-timeline)
    ...
    (define-key map (kbd "C-c s +") 'scientific-mapping-create-research-project)
    ...
    (define-key map (kbd "C-c s J") 'workflow-integration-frictionless-research-session)
    (define-key map (kbd "C-c s j") 'workflow-integration-smart-process-buffer)
    ...
    ;; Tutorial system
    (define-key map (kbd "C-c s u") 'tutorial-system-interactive-walkthrough)

    ;; Git integration (core pillar)
    (define-key map (kbd "C-c s G") 'git-integration-smart-commit)
    (define-key map (kbd "C-c s V") 'git-integration-research-history)
    (define-key map (kbd "C-c s N") 'git-integration-create-research-branch)
    (define-key map (kbd "C-c s O") 'git-integration-commit-research-milestone)
...
(defun scientific-mapping-list-key-bindings ()
  ...
  (let ((buffer (get-buffer-create "*Scientific Mapping Key Bindings*")))
    (with-current-buffer buffer
      ...
      (insert "- `C-c s q`: Quick note (title only)\n")
      (insert "- `C-c s C`: Capture via org-capture\n")
      (insert "- `C-c s y`: Capture with yasnippet (r=research, l=review, c=conference, t=thesis)\n")
      ...
      (insert "- `C-c s f`: Open org-file-manager\n")
      (insert "- `C-c s H`: Open scientific dashboard\n")
      (insert "- `C-c s l`: Start writing with integrated timeline (80/20 split)\n")
      (insert "- `C-c s L`: Toggle timeline view on/off\n")
      (insert "- `C-c s M-l`: Close timeline, return to normal view\n")
      (insert "- `C-c s u`: Start tutorial system\n\n")
      ...
      (insert "- `C-c s +`: Create research project\n")
      ...
      ;; Workflow integration
      (insert "## Workflow Integration\n")
      (insert "- `C-c s W`: Show workflow status\n")
      (insert "- `C-c s U`: Unified search\n")
      (insert "- `C-c s F`: Import paper complete\n")
      (insert "- `C-c s J`: Frictionless research session\n")
      (insert "- `C-c s j`: Smart process buffer\n\n")
